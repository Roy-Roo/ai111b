...
openai.api_key = os.getenv("OPENAI_API_KEY") # 將API key放在環境變數上傳Github時較為安全
openai.api_key = "OPENAI_API_KEY" # 將API key直接放在程式內，較不安全
...
* 使用多線程(thread)，呼叫chatGPT，把寫完的文字檔放入md檔。這邊使用slot filling，讓我們使用模板，並改裡面的關鍵字就可以。
